...
@section('title', 'Text Splitter - ' . config('app.name'))
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <nav class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div class="max-w-7xl mx-auto px-4 py-3">
            ...
            <span class="text-gray-900 dark:text-white">Text Splitter</span>
        </div>
    </nav>

    <div class="max-w-2xl mx-auto px-4 py-12">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-6">Text Splitter</h1>

            <div x-data="{ input: '', delimiter: ',', output: '' }" class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Delimiter</label>
                    <input
                        x-model="delimiter"
                        @input="output = input.split(delimiter).filter(x => x.trim()).map(x => '- ' + x.trim()).join('\n')"
